...
<head>
    ...
    <meta name="description" content="Easily convert PDF files, PowerPoint slides, and images to editable text with our free OCR tool. Fast, accurate, and ideal for extracting content from scanned documents.">
    <!-- Open Graph -->
    <meta property="og:title" content="File To Text – Convert files to editable text OCR">
    <meta property="og:description"
          content="Easily convert PDF files, PowerPoint slides, and images to editable text with our free OCR tool. Fast, accurate, and ideal for extracting content from scanned documents.">
    <meta property="og:type" content="website">
    <meta property="og:url" content="https://filetotext.online/">
    <meta property="og:image" content="https://filetotext.online/assets/og-image.png">
    ...
    <meta name="twitter:title" content="File To Text – Convert files to editable text">
    <meta name="twitter:description"
          content="Easily convert PDF files, PowerPoint slides, and images to editable text with our free OCR tool. Fast, accurate, and ideal for extracting content from scanned documents.">
    <meta name="twitter:image" content="https://filetotext.online/assets/og-image.png">


    <title>File To Text – Convert files to editable text OCR</title>

    <link rel="canonical" href="https://filetotext.online/"/>
...
<body class="bg-gray-100 min-h-screen flex items-center justify-center p-6 pt-16 md:pt-20">
<!-- Fixed Header -->
...
<main class="bg-white shadow-lg rounded-xl p-8 w-full max-w-md mx-auto" style="margin-top: var(--header-height)">
    <h1 class="text-2xl font-bold mb-6 text-gray-800 text-center" itemprop="name">
        Convert PDF, PowerPoint & Images to Text – Free OCR Tool
    </h1>

    <meta itemprop="applicationCategory" content="Utility">
    <meta itemprop="operatingSystem" content="All">
    <meta itemprop="url" content="https://filetotext.online">
    @if ($errors->any())
        <div class="mb-6 rounded-xl border border-red-300 bg-red-50 p-4 text-sm text-red-800 shadow-sm" role="alert" aria-live="assertive">
            <h2 class="mb-2 font-semibold text-red-700">Please fix the following errors:</h2>
    ...
    <form action="/upload" method="post" enctype="multipart/form-data" class="space-y-4" itemprop="potentialAction"
          itemscope itemtype="https://schema.org/UploadAction">
        @csrf

        <div x-data="{ selected: 'file' }" class="space-y-4">
            ...
            <div>
                ...
                <div class="flex rounded-xl overflow-hidden shadow-sm border border-gray-300 w-full">
                    <!-- File option -->
                    <button
                            type="button"
                            :class="selected === 'file'
                    ...
                    : 'bg-white text-gray-700 hover:bg-gray-100'"
                            class="w-1/2 px-4 py-3 text-sm font-medium focus:outline-none transition-colors duration-200"
                            @click="selected = 'file'"
                    >
                        📎 Upload File
                    </button>
                ...
                <input type="hidden" name="uploadMethod" :value="selected">
            </div>

            <!-- File input -->
            <div x-show="selected === 'file'" x-transition>
                <label for="file" class="block text-sm font-medium text-gray-700">Upload a file</label>
                <input
                        type="file"
                        name="file"
                        id="file"
                        accept=".pdf,.ppt,.pptx,.jpg,.jpeg,.png,.bmp,.tiff,.webp"
                        :required="selected === 'file'"
                        class="mt-1 w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
            </div>

            <!-- Camera input -->
            <div x-show="selected === 'camera'" x-transition>
                <label for="cameraInput" class="block text-sm font-medium text-gray-700">Take a picture</label>
                <input
                        type="file"
                        name="photo"
                        id="cameraInput"
        ...
        <button type="button"
                onclick="toggleSection('advanced-options')"
                class="flex items-center text-sm text-blue-600 hover:text-blue-800 transition-colors duration-200 focus:outline-none">
            ...
            <div class="space-y-4 border-t border-gray-200 pt-4">
                ...
                <div class="flex items-center">
                    <input
                            ...
                            {{ old('greyscale', true) ? '' : 'checked' }}
                    >
                    <label for="greyscale" class="ml-2 block text-gray-700 font-medium">
                        Apply Greyscale Processing
                    </label>
                </div>

                <!-- DPI Dropdown -->
                <div>
                    <label for="dpi" class="block text-gray-700 font-medium mb-1">
                    ...
                    >
                        @foreach ([50, 100, 150, 200, 250, 300] as $dpi)
                            <option value="{{ $dpi }}" {{ old('dpi', 100) === $dpi ? 'selected' : '' }}>{{ $dpi }}DPI
                            </option>
                        @endforeach
                    </select>
                </div>
                <!-- Contrast Slider -->
                <div>
                    <label for="contrast" class="block text-gray-700 font-medium mb-1">
                    ...
                    <input
                            ...
                            max="15"
                            step="1"
                            value="{{ old('contrast', 0) }}"
                            class="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-600"
                            aria-valuemin="-15"
                            aria-valuemax="15"
                            aria-valuenow="{{ old('contrast', 0) }}"
                            aria-label="Contrast adjustment slider from -15 (decrease) to +15 (increase)"
                    >
                    <div class="flex justify-between text-xs text-gray-600 mt-1">
                        <span>-15 (Low Contrast)</span>
                        <span>0 (No Change)</span>
                        <span>+15 (High Contrast)</span>
                    </div>
                </div>
        ...
        @php
            $defaultLanguages = old('languages', ['eng']);
        @endphp

        <div class="pl-2 border-l border-gray-200">
            <div class="max-w-md mt-2">
                <p class="text-sm text-gray-700 font-medium mb-2">
                    <strong>Select the languages that are found in your document:</strong>
                </p>

                <div class="flex items-start bg-yellow-100 border border-yellow-300 text-yellow-800 text-sm rounded-lg p-4 mb-4" role="alert">
                    <svg class="w-5 h-5 mt-0.5 mr-3 flex-shrink-0 text-yellow-500" xmlns="https://www.w3.org/2000/svg" fill="none"
                         viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M11.25 4.5l.75 10.5m0 0h.75m-.75 0H9.75M12 19.5h.01M21 12A9 9 0 113 12a9 9 0 0118 0z"/>
                    </svg>
                    <p class="font-medium">
                        The default pre-selected language is "English"
                    </p>
                </div>

                <select id="languages" class="ts-cloak" name="languages[]" multiple placeholder="Search or select languages...">
                    @foreach ($installedLanguages as $name => $code)
                        <option value="{{ $code }}"
                                {{ in_array($code, $defaultLanguages, true) ? 'selected' : '' }}>
                            {{ $name }}
                        </option>
                    @endforeach
                </select>

                <p class="text-xs text-gray-500 mt-2">You may select multiple languages.</p>
            </div>
        </div>
        ...
        <script>
            function toggleSection(id) {
                ...
                if (el.classList.contains('max-h-0')) {
                    ...
                    el.classList.add('max-h-screen', 'opacity-100');
                } else {
        ...
        <div class="flex items-center space-x-2">
            <input
                    ...
                    {{ old('consent') ? 'checked' : '' }}
            >
            <label for="consent" class="text-gray-700 font-medium">
                I agree to the <a href="{{ route('privacy.policy') }}" target="_blank" class="text-blue-600 underline">Privacy Policy</a>.
            </label>
        </div>

        <meta itemprop="encodingType" content="multipart/form-data">
        <input
                type="submit"
                value="Upload and Extract Text"
                class="w-full bg-blue-600 text-white font-semibold py-2 rounded-lg hover:bg-blue-700 transition duration-200"
                aria-label="Upload file and start text extraction"
        >
    </form>
    <p class="mt-4 text-sm text-gray-500 text-center">
        <strong>Your files are processed securely and never stored.</strong>
    </p>
    <div class="max-w-4xl mx-auto my-8 px-4">
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center shadow-sm">
            <h2 class="text-xl font-semibold text-blue-800 mb-2">
                {{ __('Want to understand your results?') }}
            </h2>
            <p class="text-blue-700 mb-4">
                {{ __('Learn how to interpret the extracted text and improve accuracy.') }}
            </p>
            <a href="{{ route('resultsExplanation') }}"
               class="inline-block bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-5 rounded transition">
                {{ __('Read the Results Guide') }}
            </a>
        </div>
    </div>
    <section class="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-2xl mt-10">
        <h3 class="text-3xl font-bold mb-4 text-gray-800">Supported File Conversion Features</h3>
        <p class="text-gray-700 mb-6">
            Easily upload your PDF or PowerPoint (PPTX) files and convert them into editable, searchable text using advanced
            <a href="https://en.wikipedia.org/wiki/Optical_character_recognition"
               class="font-medium text-blue-600 underline dark:text-blue-500 hover:no-underline">OCR (Optical Character Recognition)</a> technology.
            Perfect for extracting text from scanned documents, lecture slides, and image-based files with high accuracy.
        </p>
        <div class="space-y-6">
            <div>
                <h4 class="text-xl font-semibold text-gray-800">Greyscale Processing</h4>
                <p class="text-gray-600">
                    Activate greyscale mode to improve OCR performance on files with colorful or complex backgrounds.
                    Converting images to greyscale helps minimize distractions and enhances text clarity for more precise recognition.
                </p>
            </div>
            <div>
                <h4 class="text-xl font-semibold text-gray-800">DPI (Resolution) Selection</h4>
                <p class="text-gray-600">
                    Adjust the DPI (Dots Per Inch) to control image resolution before text extraction.
                    Higher DPI values can boost recognition of fine characters and symbols, although excessive resolution might introduce noise in poor-quality scans.
                </p>
            </div>
            <div>
                <h4 class="text-xl font-semibold text-gray-800">Language Detection</h4>
                <p class="text-gray-600">
                    Choose one or more OCR languages that match the content of your document.
                    By default, English (<code>eng</code>) is used, but selecting the correct language(s) enhances recognition accuracy across multilingual files.
                </p>
            </div>
            <div>
                <h4 class="text-xl font-semibold text-gray-800">Contrast Adjustment</h4>
                <p class="text-gray-600">
                    Fine-tune the contrast level to sharpen the distinction between text and background.
                    Higher contrast settings can improve OCR results for faded or low-contrast scans.
                    The slider ranges from 1 (lowest) to 15 (highest), with a default level that typically delivers balanced performance.
                </p>
            </div>
        </div>
    </section>
    ...
    <section class="mt-8 border-t border-gray-200 pt-6">
        <div class="flex flex-col items-center space-y-4 text-center">
            <h2 class="text-xl font-semibold text-gray-800">Support My Work</h2>
            <p class="text-gray-600 text-sm">
                If you enjoy what I build, consider buying me a coffee. Every cup fuels the next project!
            </p>
            <a
                    href="https://buymeacoffee.com/mhi_dev"
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label="Buy me a coffee"
                    class="inline-flex items-center px-4 py-2 bg-yellow-400 hover:bg-yellow-300 text-black font-medium rounded-lg shadow transition"
            >
                ☕ Buy Me a Coffee ?
            </a>
            <img
                    ...
                    class="w-20 h-20 rounded-md border border-gray-300"
                    loading="lazy"
            />
        </div>
    </section>
